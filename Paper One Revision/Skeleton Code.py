...
#Puzzle1 = data for unsolved puzzle
#Puzzle1P = data for solutions
#Puzzle1S = solutions
...
def ResetDataStructures(): #creates the puzzles grid
...
def LoadPuzzleFile(PuzzleName, Puzzle): #loads unsovled puzzle
  try:
    Line = 0
...
def LoadSolution(PuzzleName, Solution): #loads puzzle solution (checks if its done right (if = 0), also checks if file exists)
  OK = True
  try:
...
def ResetAnswer(PuzzleName, Answer): #gets rid of numbers within the grid (so the user can complete the game)
  Answer[0] = PuzzleName
  Answer[1] = "0"
  Answer[2] = "0"
...
def TransferPuzzleIntoGrid(PuzzleName, PuzzleGrid, Puzzle, Answer): #transfers read file
  OK = True
  try:
    Line = 0
...
def LoadPuzzle(PuzzleGrid, Puzzle, Answer, Solution): #placing the loaded file into the grid
...
def TransferAnswerIntoGrid(PuzzleGrid, Answer): #transfers loaded solution back into the grid
...
def LoadPartSolvedPuzzle(PuzzleGrid, Puzzle, Answer, Solution): #loads the solution file
...
def DisplayMenu(): #displays options to user
  print()
  print("Main Menu")
  print("=========")
...
def GetMenuOption(): #gets value from user for #NumberPuzzle
  Choice = EMPTY_STRING
  while len(Choice) != 1:
    Choice = input("Enter your choice: ")
  return Choice[0]

def KeepPuzzle(PuzzleGrid, Answer): #writes the user's solution to the puzzle file
  if PuzzleGrid[0][0] != 'X':
    print("No puzzle loaded")
  else:
    if int(Answer[2]) > 0:
...
def CheckSolution(PuzzleGrid, Answer, Solution): #compares the user's solution with the actual solution
  ErrorCount = 0
  Solved = False
  Correct = True
  Incomplete = False
...
def NumberPuzzle(): #gets the user's input and acts upon it, does error checking and proides answer
  Finished = False 
  PuzzleGrid, Puzzle, Answer, Solution = ResetDataStructures()
  while not Finished:
    DisplayMenu()
...
if __name__ == "__main__": #triggers the code (tells python which subroutine to trigger)
  NumberPuzzle()