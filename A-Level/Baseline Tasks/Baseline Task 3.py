...
if secretword.isalpha():
    if len(secretword)>=6 and len(secretword)<=12:
            print("Player 2 you must guess the secret word")
            print("The word has", len(secretword), "letters")
    else:
            print("ERROR: secret word is not long enough (between six and 12 letters)")    
else:
    print("ERROR: secret word contains numbers (letters only)")
#Game
while loop==False:
