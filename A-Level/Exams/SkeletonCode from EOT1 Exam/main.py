...
def MirrorImage(Grid, Header):
  NewGrid = [[EMPTY_STRING for Column in range(MAX_WIDTH)] for Row
in range(MAX_HEIGHT)]
  NewGrid = ClearGrid(NewGrid)
  for ThisRow in range(Header.Height):
    for NewColumn in range(Header.Width):
      NewGrid[ThisRow][NewColumn] = Grid[ThisRow][Header.Width - NewColumn - 1]
...
def FindSecretChar(PixelValue, Key):
  ...
  if NewValue == 0:
    Character = ' '
  elif NewValue in range(1, 27):
    Character = chr(ord('A') + NewValue - 1)
  return Character

def LoadGreyScaleImage(FileIn, Grid, Header):
  try:
    Key = int(Header.Title[-1])
